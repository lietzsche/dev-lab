...
- 핵심 구현은 학습자가 작성한다. 새 단원의 실행 골격, 설정, 반복적인 test fixture 같은 boilerplate는 Codex가 먼저 작성한다.
...
새 소단원을 시작할 때는 학습자가 핵심 개념 구현에 바로 집중할 수 있도록 Codex가 실행 가능한 골격을 먼저 준비한다.
...
- 학습자의 핵심 구현을 임의로 고치지 않는다. 반복적인 정리나 test fixture를 Codex가 수정할 때는 무엇을 바꿨는지 알려준다.
