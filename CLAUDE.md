...
- 최신 완료 단원: `P9-3` FastAPI application — 상태: 완료
- 다음 단원: `P9-4` API test와 운영 경계 (사용자가 명시적으로 `넘어가자`라고 요청하기 전에는 시작하지 않는다)
- 최신 완료 단원: `P10-1` configuration과 secret — 상태: 완료
- 다음 단원: `P10-2` dependency boundary (사용자가 명시적으로 `넘어가자`라고 요청하기 전에는 시작하지 않는다)
- 이 스냅샷은 세션 시작 시점의 참고용이며, 항상 `PROGRESS.md`의 `## 현재`, `## 현재 작은 단계` 절을 정본으로 다시 확인한다.
...
- 핵심 구현은 학습자가 작성한다. 새 단원의 실행 골격, 설정, 반복적인 test fixture 같은 boilerplate는 Claude가 먼저 작성한다.
...
새 소단원을 시작할 때는 학습자가 핵심 개념 구현에 바로 집중할 수 있도록 Claude가 실행 가능한 골격을 먼저 준비한다.

- 새 lesson module을 만들고 `run()` 함수, 빈 줄 출력용 `print()`, 단원 시작 메시지를 작성한다.
- `src/knowledge_lab/__main__.py`에 새 module을 import하고 현재 실행 대상으로 연결하며, 이전 lesson의 `run()` 호출은 주석 처리한다.
- 현재 단원을 확인하는 smoke test의 import와 시작 메시지를 갱신한다.
- 골격이 정상 실행되고 기존 test가 통과하는지 확인한 뒤 첫 개념과 학습자 구현을 안내한다.
- 파일 생성, import 연결, 이전 단원 주석 처리, 반복적인 시작 메시지와 smoke test 수정은 학습자 과제로 주지 않는다.
- 미래 개념의 함수, class, directory는 미리 만들지 않는다. 현재 개념에 필요한 최소 골격만 준비한다.

## 구현 과제 안내

학습자에게 구현을 요청할 때는 다음 정보를 빠뜨리지 않는다.

1. 수정할 파일
2. 새로 작성하거나 변경할 함수와 class의 이름 및 signature
3. 구현해야 할 동작과 이번 단계에서 다루지 않을 문법
4. 코드를 연결하거나 호출할 위치
5. 실행 명령과 관찰할 결과

- 완성 코드를 먼저 제공하지 않되, 학습자가 핵심 개념이 아닌 연결 작업을 추측하게 만들지 않는다.
- 대화 속 최소 예제의 이름이 과제의 실제 이름과 다르면 예제 전용임을 밝히고, 학습자의 실제 파일에 예제 이름이 남지 않았는지 확인한다.
- 이전 실험 호출이 남아 중복 출력되거나 새 관찰을 방해하지 않도록 다음 개념으로 넘어가기 전에 정리한다.
...
- 채팅에 붙여 넣은 코드는 Markdown 변환으로 공백이나 기호가 달라질 수 있으므로 가능한 경우 저장소의 실제 파일을 확인한다.
- 먼저 실행 결과와 개념 이해를 확인하고, 그다음 오타, 들여쓰기, 사용되지 않는 이름, 불필요한 분기 같은 수정 사항을 구체적인 위치와 함께 안내한다.
- 요청한 수정이 실제 파일에 반영됐는지 확인하기 전에는 해당 개념을 완료로 기록하거나 다음 개념으로 넘어가지 않는다.
- 학습자의 핵심 구현을 임의로 고치지 않는다. 반복적인 정리나 test fixture를 Claude가 수정할 때는 무엇을 바꿨는지 알려준다.
...
- 명령과 문서에서는 저장소 폴더 이름을 고정해서 가정하지 않고 Git 저장소 루트를 기준으로 상대 경로를 사용한다.
- 전달받은 작업 directory가 존재하지 않으면 shell이나 권한 문제로 단정하지 않는다. 먼저 상위 경로와 실제 Git root를 확인해 저장소 이동 또는 이름 변경 여부를 구분한다.
- 실제 Git root를 찾으면 그 위치에서 정본 문서, status, remote를 다시 확인한 뒤 작업을 계속한다.
...
이 script는 source를 수정하지 않고 Ruff formatter, Ruff linter, mypy, pytest를 차례로 실행한다. 하나라도 실패하면 즉시 non-zero status로 종료한다. 소단원을 완료로 표시하기 전에 반드시 통과시킨다.
